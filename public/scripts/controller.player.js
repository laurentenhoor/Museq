	...
	museq.Player = function(beat) {
		...
		this.createInstruments = function() {
			...
			for (var i = 0; i < instrumentsConfig.length; i++) {
				...
				instrument.initialize(function() {
					// Loading Ready Callback
				});
		...
		function getFirstNoteIndex() {
			
			var smallestIndex = _instruments[0].tracks[0].notes.length;
			
			for (var i = 0; i < _instruments.length; i++) {
				for (var j = 0; j < _instruments[i].tracks.length; j++) {
					for (var k = 0; k < _instruments[i].tracks[j].notes.length; k++) {

						if (_instruments[i].tracks[j].notes[k] == 1) {
							if (k < smallestIndex) {
								smallestIndex = k
								if (smallestIndex == 0) {
									return 0;
		...
		this.startTwoBeats = function(stopCallback) {
			...
			// on mobile a touch event should immediately start audio
			_noteIndex = getFirstNoteIndex();	
			
			console.log('NoteIndex: ' + _noteIndex);
...
//			if (_started) return;
		...
		this.schedule = function() {
			...
			while (_noteTime < (currentTimeDraw + 0.200)) {
		...
		this.loadBeat = function() {
			...
			$.each(instruments, function(instrument_key, instrument) {
				$.each(instrument.tracks, function(track_key, track) {
					_instruments[instrument_key].tracks[track_key].notes = track.notes.slice();
